...
public final class Client {
	 ...
	 * @throws A CKANException if the request fails
	 */
	public DataStore createDataStore(DataStore datastore) throws CKANException {
		Gson gson = new Gson();
	...
	/**
	 * Retrieves a datastore table
	 *
	 * Retrieves a datastore table with the given ID or alias from CKAN
	 *
	 * @param id		The id or alias to search
	 * @returns A SearchResults object that contains a count and the objects
	 * @throws A CKANException if the request fails
	 */
	public DataStore getDataStore(String id) throws CKANException {
		String returned_json = this._connection.Post("/api/action/datastore_search", "{\"id\":\"" + id + "\"}");
		DataStore.Response r = LoadClass(DataStore.Response.class, returned_json);
		if (!r.success) {
			HandleError(returned_json, "getDataStore");
		}
		return r.result;
	}
	 ...
	 *
	 * Deletes a datastore table specified with the provided ID
	 *
	 * @param id		The ID of the datastore table to delete
	 * @throws A CKANException if the request fails
	 */
	public void deleteDataStore(String id) throws CKANException {
		String returned_json = this._connection.Post("/api/action/datastore_delete", "{\"id\":\"" + id + "\"}");
		DataStore.Response r = LoadClass(DataStore.Response.class, returned_json);
		if (!r.success) {
			HandleError(returned_json, "deleteGroup");
		}
